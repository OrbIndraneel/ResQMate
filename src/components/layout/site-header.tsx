  ...
  return (
    <header className="px-8 h-24 flex items-center nav-blur sticky top-0 z-50">
      <div className="flex items-center gap-6">
        <Link className="flex items-center gap-3 group" href={userRole === 'ngo' ? '/ngo/dashboard' : '/volunteer/dashboard'}>
      ...
      <div className="ml-auto flex items-center gap-5">
        ...
        <Button variant="ghost" size="icon" className="relative h-12 w-12 rounded-2xl bg-slate-50 hover:bg-slate-100">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="relative h-12 w-12 rounded-2xl p-0.5 border-2 border-transparent hover:border-primary/20">
          ...
          <DropdownMenuContent className="w-72 rounded-[2rem] p-3 mt-4 shadow-2xl border-none glass-panel" align="end">
            <DropdownMenuLabel className="font-normal p-5">
              <div className="flex flex-col space-y-2">
                <p className="text-lg font-black leading-none text-slate-950 truncate">{userName}</p>
            ...
            <div className="p-2 space-y-1">
              <DropdownMenuItem className="rounded-2xl py-4 px-5 font-bold cursor-pointer" asChild>
              ...
              <DropdownMenuItem className="rounded-2xl py-4 px-5 font-bold cursor-pointer" asChild>
                <Link href={userRole === 'ngo' ? '/ngo/profile' : '/volunteer/profile'}>
                  <Settings className="mr-4 h-5 w-5 text-slate-400" /> Settings
                </Link>
              </DropdownMenuItem>
            </div>
            <DropdownMenuSeparator />
            <div className="p-2">
              <DropdownMenuItem className="rounded-2xl py-4 px-5 font-bold cursor-pointer text-rose-500 hover:bg-rose-50" onClick={() => router.push('/')}>
